...
describe('Auth Controller', () => {
    ...
    describe.only('requestResetPassword', () => {
        ...
        it.only('should send an email and return success message', async () => {
            //not working
            // const user = { _id: 'userID' };
            // const mockFindUser = jest.spyOn(User, 'findOne');
            // mockFindUser.mockResolvedValue(user);
            // jest.spyOn(authModule, 'generateResetPWToken').mockImplementation(
            //     () => 'token'
            // );
            ...
            // });
            const req = { body: { email: 'ape@ape.com' } };
            const res = {
                status: jest.fn().mockReturnThis(),
                json: jest.fn(),
            };
            jest.spyOn(User,'findOne').mockResolvedValue({ id: '123' });
            jest.spyOn(authModule,'generateResetPWToken').mockResolvedValue('token');
            await requestResetPassword(req, res);
